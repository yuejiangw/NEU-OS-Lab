...
void produce()
{
	if(findEmpty(buffer, BUFFERSIZE) == -1) // 缓冲区满,则阻塞
	{
		++data;
		producer++;
		pQueue.push(data);
	}
	else if(isEmpty(buffer, BUFFERSIZE)) // 缓冲区空
	{
		if(cQueue.empty())  // 消费者等待队列为空,直接写入即可
		{
			data++;
...
void consume()
{
	if(findDirty(buffer, BUFFERSIZE) == -1) // 缓冲区空,则阻塞
	{
		consumer++;
		cQueue.push(data);
	}
	else if(isFull(buffer, BUFFERSIZE)) // 缓冲区满
	{
